...
namespace PasswordManager.Classes
{
    ...
    public class Passwords
    {
        ...
        public List<PasswordData> Find(string? type = null, string? username = null) =>
            PasswordsList.Where(item =>
                    (string.IsNullOrWhiteSpace(type) || item.PasswordType.Contains(type, StringComparison.OrdinalIgnoreCase)) &&
                    (string.IsNullOrWhiteSpace(username) || item.Username.Contains(username, StringComparison.OrdinalIgnoreCase))
            ).ToList();
